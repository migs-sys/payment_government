...
#[contract]
pub struct ProofOfPaymentForGovernmentServicesContract;

#[contractimpl]
impl ProofOfPaymentForGovernmentServicesContract {
    ...
    pub fn pay_public_sector_record(env: Env, id: String, actor: Address, amount: i128, score: u32) -> Result<(), Error> {
        if amount <= 0 {
            return Err(Error::InvalidAmount);
        }
